...
Программа принимает два аргумента командной строки: входной файл и выходной файл.

Обработка логов:

Читает входной файл построчно

Для каждой строки пытается определить формат (Format 1 или Format 2)

Преобразует найденные данные в стандартный формат

Преобразования:

Дата: Конвертируется в формат DD-MM-YYYY
...
Остальные уровни остаются без изменений

Метод: Если отсутствует, используется "DEFAULT"

Сообщение: Переносится без изменений

Формат вывода:

Поля разделяются табуляцией

Порядок полей: Дата, Время, Уровень логирования, Вызвавший метод, Сообщение

Обработка ошибок:

Если строка не соответствует ни одному из форматов, выводится предупреждение

Основные ошибки обрабатываются в блоке try-catch

Пример использования:
LogStandardizer input.log output.log
